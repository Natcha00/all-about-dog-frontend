 ...
 * Body: { email: string }
 * Proxies to POST /dog-owner/forgot-password. Sends OTP to email.
 */
export async function POST(request: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${base}/dog-owner/forgot-password`, {
      method: "POST",
  ...
  const message = (data.message as string) || (data.error as string) || "ขอ OTP ไม่สำเร็จ";
